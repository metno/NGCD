      ...
      integer :: i,j,ii,io,jj,n,tri,point_within_how_many_triangles,triangle_num,node_num,it,stnr,inint
	  
      integer (kind=4 ), dimension(2020,2) :: x_grid_indices_norge
      integer (kind=4 ), dimension(1550,2020) :: point_within_triangle,buffer
      ...
      real ( kind = 8 ), dimension (1550) :: x
      real ( kind = 8 ), dimension (2020) :: y
      ...
      real ( kind = 8 ), dimension(1550,2020)  :: rr_grd,z_amsl_grd,dem1km,rr1km
      real ( kind = 8 ), dimension(1550,2020)  :: z_amsl_grd_new,rr_grd_new
      real ( kind = 8 ) :: x1,x2,x3,h1,h2,h3,Denom,y1,y2,y3,d_rr_over_d_x,d_rr_over_d_y,d_z_over_d_x,d_z_over_d_y,rr1,rr2,rr3
      character cdum*1	  
          
!---- Read daily precipitation data from file "rr.par" ----	  	  
      zfac=0.001
...
!---- Read triangle nodes computed by Geompack ----	  
      ...
      open(1,file='triangle_nodes.dat',status='old')
      do j=1,10000
        read(1,*,iostat=io)(triangle_nodes_index(i,j),i=1,3)
        if(io<0) exit			
        write(99,*)(triangle_nodes_index(i,j),i=1,3)      
      enddo
      close(1)
      triangle_num=j-1
      write(*,*)'Number of triangles = ', triangle_num

!---- Create grid for seNorge ----	  

      do i=1,1550
        x(i)=float(i-1)*1000. + 4000000. + 500.
      enddo
      do j=1,2020 
        y(j)=float(j-1)*1000. + 3410000. + 500.
      enddo
      print*,'Grid created'

      !open(1,file='../temperatur/fenno_dem_u33.asc',status='old') ! --> NB CHANGE THIS BACK AFTER TESTING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      open(1,file='dem1km.asc',status='old')	  
      do i=1,6
        read(1,*)
      enddo
      do j=2020,1,-1
      	read(1,*)(dem1km(i,j),i=1,1550)
      enddo
      close(1)

      open(1,file='fennomask.asc',status='old')	  
      do i=1,6
        read(1,*)
      enddo
      do j=2020,1,-1
      	read(1,*)(buffer(i,j),i=1,1550)
      enddo
      close(1)
...
!---- Set coordinates of nodes 1, 2 and 3 of current triangle ----
            ...
            if (i == 590) then
               if (j == 590) then
                  write(98,'(i5,4f12.1)')tri,x(i),x_triangle(1),x_triangle(2),x_triangle(3)
                  write(98,'(i5,4f12.1)')tri,y(i),y_triangle(1),y_triangle(2),y_triangle(3)
               endif
            endif
...
33333       continue
          ...
          if(mod(j,100)==0 .and. mod(i,100)==0) then
            write(99,*)i,j,point_within_triangle(i,j),point_within_how_many_triangles
          endif
          if(point_within_how_many_triangles > 1) then
            write(99,*)i,j,point_within_how_many_triangles
            write(99,*)'>>>>>>>>> SOMETHING IS WRONG! CHECK PROGRAM! <<<<<<<<<'
            !stop
          endif
...
!      write(*,'(a20)')'Return to continue: '
!      read(*,'(a1)')cdum

      open(1,file='point_within_triangle_number.dat')
      do j=2020,1,-1
        write(1,'(1550i5)')(point_within_triangle(i,j),i=1,1550)
      enddo
      close(1)
...
!----
    ...
    print*,'Point in triangle complete'
      ...
      open(1,file='rr2_grd.asc')
      write(1,'(a)')'ncols   1550'
      write(1,'(a)')'nrows   2020'
      write(1,'(a)')'xllcorner  4000000'
      write(1,'(a)')'yllcorner  3410000'
      write(1,'(a)')'cellsize  1000'
      write(1,'(a)')'NODATA_value -999'
      do j=2020,1,-1
      	write(1,'(1550f6.1)')(rr_grd(i,j),i=1,1550)		
      enddo
      close(1)


	  
      open(1,file='z_grd.asc')
      write(1,'(a)')'ncols   1550'
      write(1,'(a)')'nrows   2020'
      write(1,'(a)')'xllcorner  4000000'
      write(1,'(a)')'yllcorner  3410000'
      ...
      write(1,'(a)')'NODATA_value -999'
      do j=2020,1,-1
      	write(1,'(1550f7.1)')(z_amsl_grd(i,j),i=1,1550)
      enddo
      close(1)
...
!---- Use nearest neighbour for points of Norway that are not covered by any triangles ----
      write(99,*)'Fill points of Norway that are not covered by any triangles with nearest neighbour:' 
      do i=1,1550
        do j=1,2020
          rr_grd_new(i,j)=-999			
          ...
          if(rr_grd(i,j)==-999 .and. dem1km(i,j)>=0.) then		  
            ...
            do ii=max(i-100,1),min(i+100,1550)       !  --> Find closest neighbour from triangulated precipitation field (by going 100 km to west, east, north and south)
            do jj=max(j-100,1),min(j+100,2020)	
      ...
      do i=1,1550
        do j=1,2020
          ...
          if (rr_grd(i,j)>=0) write(42,'(2i10,f10.2,f8.1)')(i*1000)+4000000,(j*1000)+3410000,rr_grd(i,j),z_amsl_grd_new(i,j)
        enddo
     enddo
      write(99,'(i10,a)')n,' points were outside triangles. These are filled with values from nearest triangle!'
      write(*,'(i10,a)')n,' points were outside triangles. These are filled with values from nearest triangle!'	  
      ...
      do i=1,1550
        do j=1,2020     
          ...
          if(buffer(i,j) < -99 .or. z_amsl_grd(i,j) < 0) cycle ! --> "Cycle" means jump to next execution of loop ("do" command) by increasing i or j.
...
!---- Write final gridded and terrain-corrected precipitation file ----
      ...
      write(1,'(a)')'NODATA_value -999'
      do j=2020,1,-1
        write(1,'(1550f7.1)')(rr1km(i,j),i=1,1550)
!---      	write(1,'(1550f7.1)')(nint(rr1km(i,j)),i=1,1550)
      enddo
      close(1)
