...
dyn.load(paste(main.path,"/Bspat_PREC1d/src/oi_first.so",sep=""))
dyn.load(paste(main.path,"/Bspat_PREC1d/src/oi_fast.so",sep=""))
# CG to FG, faster! 
file.cg2fg<-paste(main.path.geoinfo,"/fennodem10000to1000_easy.dat",sep="")
...
#        xtmp.FG[]<-na1
#        xtmp.FG[xindx.eve.FG]<-1
#        xtmp.FG<-CG2FG_bilinear(cg2fg,x.FG=xtmp.FG,x.CG=xc,na.value=na1)
#        xf[xindx.eve.FG]<-xtmp.FG[xindx.eve.FG]
#        r.aux.FG[]<-NA
#        r.aux.FG[mask.FG]<-round(xf,1)
#      } else {
#        r.aux.FG[mask.FG]<-round(xa.FG,1)
#      }
#      nogrid.ncout(file.name=paste("./scale",
#                                   "_n",formatC(n,width=3,flag="0",format="d"),
#                                   "_Dh",formatC(Dh.test,width=5,flag="0",format="d"),
#                                   "_Dz",formatC(Dz.test,width=5,flag="0",format="d"),".nc",sep=""),
#                   grid=t(as.matrix(r.aux.FG)),
#                   x=x.FG,y=y.FG,grid.type=grid.type,
#                   times=c(paste(yyyymmdd.b,"0000",sep="")),
#                   prod.date=prod.date,
