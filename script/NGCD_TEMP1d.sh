...
# COMMAND LINE:
# =============
#  >NGCD_TEMP1d.sh -s yyyy.mm.dd  (date start)
...
#------------------------------------------------------------------------------
# log
  echo "NGCD_TEMP1d.sh "`date +%Y-%m-%d" "%H:%M`" > elaborations from "$DATESTART" UTC to "$DATEEND" UTC"
...
# Variables
  ...
  BLACKL=$MAINDIR/etc/blacklists/NGCD_TEMP1d_blacklist.txt
  ERROBS=$MAINDIR/etc/suspect_observations/NGCD_TEMP1d_suspect_observations.txt
#--------------------------------------------------
#  Clean temporary directories, if needed 
#--------------------------------------------------
#  echo "NGCD_TEMP1d.sh "`date +%Y-%m-%d" "%H:%M`" > clean directories"
...
# Exit 
#--------------------------
  echo "NGCD_TEMP1d.sh "`date +%Y-%m-%d" "%H:%M`" > success!"
  exit 0
