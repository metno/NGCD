...
# COMMAND LINE:
# =============
#  >NGCD_PREC1d.sh -s yyyy.mm.dd  (date start)
...
#------------------------------------------------------------------------------
# log
  echo "NGCD_PREC1d.sh "`date +%Y-%m-%d" "%H:%M`" > elaborations from "$DATESTART" UTC to "$DATEEND" UTC"
...
# Variables
  ...
  BLACKL=$MAINDIR/etc/blacklists/blacklist_empty.txt
  ERROBS=$MAINDIR/etc/blacklists/blacklist_empty.txt
#--------------------------------------------------
#  Clean temporary directories, if needed 
#--------------------------------------------------
#  echo "NGCD_PREC1d.sh "`date +%Y-%m-%d" "%H:%M`" > clean directories"
...
#-----------------------------------------------------
  ...
  do
    ...
    echo "================================================================================"
    echo "`date +%Y-%m-%d" "%H:%M` > $R --vanilla $DATEcur $DATEcur $BLACKL $ERROBS $CONFIG_FILE $CONFIG_PAR < $Bspat > $LOGDIR/Bspat_PREC1d_$DATEcur_$DATEcur.log 2>&1"
    $R --vanilla $DATEcur $DATEcur $BLACKL $ERROBS $CONFIG_FILE $CONFIG_PAR < $Bspat > $LOGDIR/Bspat_PREC1d_$DATEcur_$DATEcur.log 2>&1
    SECcur=$(( SECcur+86400 ))
  done
#--------------------------
# Exit 
#--------------------------
  echo "NGCD_PREC1d.sh "`date +%Y-%m-%d" "%H:%M`" > success!"
  exit 0
