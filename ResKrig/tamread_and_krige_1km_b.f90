      program tamread_and_krige
	  
      implicit none
	  
      real*8 AREA(1200),PERIMETER(1200),ST_TMP2_(1200),ST_TMP2_ID(1200),STNR(1200),TAM(1200),IT(1200)
      real*8 X(1200),Y(1200),HOH(1200),FENNOMEAN(1200),FENNO_MIN4(1200),FENNO_LAT(1200),FENNO_LONG(1200)
      real*8 VALUE0(1200),POLYGONID(1200),SCALE(1200),ANGLE(1200),r,dxy
      real*8 tam_mean,value0_mean,sum_tam,sum_value0,tam_std,value0_std
      real*8 corrcoef_y_tam,y_std,y_mean,sum_y
      real*8 corrcoef_x_tam,x_std,x_mean,sum_x,yyyyy
      real*4 variogram_sill(1),variogram_range(1), ang(1),anis(1)
      real*4 xin(1200),yin(1200),value0in(1200),out(1550,2020),outvar(1550,2020)
      integer*4 i,j,io,n,n_stations,x_grid_indices_norge(2020,2),hh,dd,mm,yyyy
      integer variogram_type(1)
      character*18 tamfilename
      character*10 c1,c2,c3
           
 
!---- Semivariogram parameters are now defined as arguments.
!      print*,'Hei'

      open(11,file="semvar.par")
      read(11,*)variogram_type,variogram_range,variogram_sill
      
      write(99,*)'--> Now running tamread_and_krige.f90'
...
!--- Read values ---
      ...
      open(1,file='Value0.dat')
      read(1,*)
10    continue
        n=n+1
        read(1,*,end=20)STNR(n),TAM(n),X(n),Y(n),HOH(n),FENNOMEAN(n),fenno_min4(n),FENNO_LAT(n),FENNO_LONG(n),VALUE0(n)
        write(99,'(2i11,f8.1,f8.1)')int(x(n)),int(y(n)),TAM(n),VALUE0(N)
      goto 10
20    continue
      close(1)
      n_stations=n-1
      write(*,*)'NUMBER OF STATIONS USED FOR KRIGING = ', n_stations	  
      if(n_stations<150) then
        open(61753,file='Dates TAMRR not defined.txt',access='append')
!        write(61753,*)yyyy,mm,dd
        write(61753,*)'NUMBER OF STATIONS USED FOR KRIGING = ', n_stations	  	
        close(61753)
      endif		
	  
!--- Run kriging ---                                                                                     
                                                                                                         
      write(99,*)'Use semivariogram parameters for monthly mean temperatures from Tveito et al. (2000):' 
      write(99,*)variogram_type,variogram_range,variogram_sill                               
                                                                                                         
      do i=1,n_stations		                                                                             
        xin(i)=x(i)                                                                                  
        yin(i)=y(i)                                                                                      
        value0in(i)=value0(i)   
 !       write(*,*)xin(i),yin(i),value0in(i)                                                                         
      enddo                                                                                              
      open(19,file='okb2d.log')
      ang(1)=0.
      anis(1)=1.
      call okb2d(n_stations,xin,yin,value0in,1550,2020,4000.500,3410.500,1.,1.,1,1,1,16,200000.,1,0., &
      variogram_type,variogram_range,variogram_sill,ang,anis,1,19,out,outvar)
      ...
      open(1,file='TN0.asc')																
      write(1,'(a12)')'ncols   1550'                                   
      write(1,'(a12)')'nrows   2020'                                   
      write(1,'(a18)')'xllcorner  4000000'                             
      write(1,'(a18)')'yllcorner  3410000'                           
      write(1,'(a14)')'cellsize  1000'                                 
      write(1,'(a20)')'NODATA_value -999.00'            
      do j=2020,1,-1                                              
        write(1,'(1550f9.2)')(out(i,j),i=1,1550)
                         
      enddo                                                       
      close(1)                                                   
   
      end program tamread_and_krige		                                            
                                                              

!---- Include required subroutines ----      									                                            

!      include 'okb2d.f90' ! Include "Geostatistical Software Library" routine "Ordinary Kriging of a 2-D Rectangular Grid"
